...
func main() {
	...
	for i := 0; i < len(scriptBytes); {
		...
		if op >= OP_DATA_1 && op <= OP_DATA_75 {
			...
			fmt.Println(opcodeArray[op].name)
			...
			fmt.Println(fmt.Sprintf("OP_PUSHDATA1(%d)", length))
			fmt.Println(hex.EncodeToString(data))
			i += 1 + int(length)
			...
			fmt.Println(fmt.Sprintf("OP_PUSHDATA2(%d)", length))
			fmt.Println(hex.EncodeToString(data))
			i += 3 + int(length)
			...
			fmt.Println(fmt.Sprintf("OP_PUSHDATA4(%d)", length))
			fmt.Println(hex.EncodeToString(data))
			i += 5 + int(length)
		} else {
			fmt.Println(opcodeArray[op].name)
